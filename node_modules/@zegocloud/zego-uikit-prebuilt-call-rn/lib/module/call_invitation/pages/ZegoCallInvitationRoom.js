...
import { ONE_ON_ONE_VIDEO_CALL_CONFIG, ONE_ON_ONE_VOICE_CALL_CONFIG, GROUP_VIDEO_CALL_CONFIG, GROUP_VOICE_CALL_CONFIG, ZegoCallEndReason } from '../../services/defines';
...
export default function ZegoUIKitPrebuiltCallInCallScreen(props) {
  ...
  const requireDefaultConfig = data => {
    const callConfig = data.invitees.length > 1 ? ZegoInvitationType.videoCall === data.type ? GROUP_VIDEO_CALL_CONFIG : GROUP_VOICE_CALL_CONFIG : ZegoInvitationType.videoCall === data.type ? ONE_ON_ONE_VIDEO_CALL_CONFIG : ONE_ON_ONE_VOICE_CALL_CONFIG;
    return {
      ...callConfig
    };
  };
  ...
  const hangUpHandle = () => {
    ...
    if (inviter === userID && CallInviteStateManage.isAutoCancelInvite(invitationID)) {
  ...
  return /*#__PURE__*/React.createElement(ZegoUIKitPrebuiltCall, {
    appID: appID,
    appSign: appSign,
    userID: userID,
    userName: userName,
