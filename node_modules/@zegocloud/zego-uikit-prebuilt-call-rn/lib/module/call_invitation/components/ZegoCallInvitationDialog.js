...
import ZegoUIKit, { ZegoAcceptInvitationButton, ZegoRefuseInvitationButton } from '@zegocloud/zego-uikit-rn';
...
export default function ZegoCallInvitationDialog(props) {
  ...
  const isDialogVisableRef = useRef(false);
  ...
  const dismissForegroundNotification = () => {
    zloginfo('[ZegoCallInvitationDialog] dismissForegroundNotification');
  ...
  useEffect(() => {
    if (isInit) {
      ...
      const callbackID = 'ZegoCallInvitationDialog' + String(Math.floor(Math.random() * 10000));
      ZegoUIKit.getSignalingPlugin().onInvitationReceived(callbackID, async ({
        callID: invitationID,
        type,
        inviter,
        data
        ...
        if (onCall || !offlineData && onRoom) {
          ...
          ZegoUIKit.getSignalingPlugin().refuseInvitation(inviter.id, JSON.stringify({
            callID: invitationID,
            reason: 'busy',
          ...
          CallEventNotifyApp.getInstance().notifyEvent('onIncomingCallRejectedCauseBusy', dataParsed.call_id, {
            userID: dataParsed.inviter.id,
            userName: dataParsed.inviter.name
          });
        } else {
          const currentData = JSON.parse(data);
          CallEventNotifyApp.getInstance().notifyEvent('onIncomingCallReceived', currentData.call_id, {
          ...
          if (offlineData && offlineData.call_id === currentData.call_id && offlineData.inviter.id === currentData.inviter.id) {
            ...
            zloginfo(`[ZegoCallInvitationDialog][onInvitationReceived] received online call, currentState: ${AppState.currentState}`);
            if (AppState.currentState === 'background' || AppState.currentState === 'inactive') {
              zloginfo('[ZegoCallInvitationDialog][onInvitationReceived] show background online notification');
            ...
            zloginfo('[ZegoCallInvitationDialog][onInvitationReceived] show foreground online notification');
            ...
            // Unable to receive timeout due to network issues; added local timeout to turn off incoming call reminder.
            let timeout = (currentData.timeout ?? 60) + 5;
            zloginfo(`[ZegoCallInvitationDialog][onInvitationReceived] will config local timeout process after ${timeout} seconds, callID: ${invitationID}`);
            setTimeout(() => {
              zloginfo(`[ZegoCallInvitationDialog][onInvitationReceived] local timeout process, current callID: ${zimCallIDRef.current}, isDialogVisable: ${isDialogVisableRef.current}`);
              if (isDialogVisableRef.current && zimCallIDRef.current === invitationID) {
                dismissForegroundNotification();
                NotificationHelper.getInstance().dismissNotification(invitationID, 'Timeout', TAG);
              }
            }, timeout * 1000);
          }
        }
      ...
      ZegoUIKit.getSignalingPlugin().onInvitationTimeout(callbackID, ({
        ...
        CallEventNotifyApp.getInstance().notifyEvent('onIncomingCallTimeout', dataInDataParsed.call_id, dataParsed.inviter);
      ...
      ZegoUIKit.getSignalingPlugin().onInvitationCanceled(callbackID, ({
        ...
        if (ZegoPrebuiltPlugins.getLocalUser().userID != inviter.id) {
          CallEventNotifyApp.getInstance().notifyEvent('onIncomingCallCanceled', dataParsed.call_id, dataParsed.inviter);
        }
  ...
  useEffect(() => {
    isDialogVisableRef.current = isDialogVisable;
  }, [isDialogVisable]);
  return /*#__PURE__*/React.createElement(View, {
    ...
    style: styles.mask
  }), !isFullScreen ? /*#__PURE__*/React.createElement(TouchableOpacity, {
    onPress: pressHandle,
    activeOpacity: 0.9
  }, /*#__PURE__*/React.createElement(View, {
    style: styles.dialog
    ...
    style: styles.avatar
  }, !avatarBuilder ? /*#__PURE__*/React.createElement(Text, {
    style: styles.nameLabel
  }, getShortName(inviter.name), " ") : /*#__PURE__*/React.createElement(View, {
    style: {
      width: '100%',
      height: '100%',
      alignItems: 'center',
    ...
    numberOfLines: 1,
    ellipsizeMode: "tail"
  }, getDialogTitle()), /*#__PURE__*/React.createElement(Text, {
    style: styles.callTitle
  }, getDialogMessage()))), /*#__PURE__*/React.createElement(View, {
    style: styles.right
  }, showDeclineButton ? /*#__PURE__*/React.createElement(View, {
    style: styles.refuse
  }, /*#__PURE__*/React.createElement(ZegoRefuseInvitationButton, {
    ...
    style: styles.accept
  }, /*#__PURE__*/React.createElement(ZegoAcceptInvitationButton, {
  ...
  }, InnerTextHelper.instance().getInnerText().incomingCallPageDeclineButton)) : null, /*#__PURE__*/React.createElement(View, {
  ...
  }, InnerTextHelper.instance().getInnerText().incomingCallPageAcceptButton))))));
}
