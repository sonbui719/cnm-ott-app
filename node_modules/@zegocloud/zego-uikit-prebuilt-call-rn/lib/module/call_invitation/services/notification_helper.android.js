...
export default class NotificationHelper {
  ...
  _displayIncomingCall(callID, callerName, message, timeout, appState) {
    var areEnabledPromise = Platform.Version < 33 ? ZegoUIKitPrebuiltCallRNModule.areNotificationsEnabled() : PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS);
  ...
  _offlineRefuse(callID, roomID, inviter, reason) {
    ...
    ZegoPrebuiltPlugins.loadLoginInfoFromLocalEncryptedStorage().then(loginInfo => {
      zloginfo(`[NotificationHelper][_onCallKitEndCall] will refuse after signalingPlugin login succ, callbackID: ${this._callbackID}`);
      ZegoPrebuiltPlugins.init(loginInfo.appID, loginInfo.appSign, loginInfo.userID, loginInfo.userName, this._plugins).then(() => {
  ...
  _onCallKitAnswerCall() {
    ...
    this._callData = undefined;
    let app_state = isPureOfflinePush ? "restarted" : AppState.currentState;
