...
export default class ZegoUIKitPrebuiltCallInvitation {
  ...
  sendCallInvitation(invitees, isVideoCall, navigation, options) {
    ...
    const nonNullNotificationMessage = notificationMessage ?? InnerTextHelper.instance().getIncomingCallDialogMessage(isVideoCall ? ZegoInvitationType.videoCall : ZegoInvitationType.voiceCall, invitees.length);
    ...
    return new Promise((resolve, reject) => {
      ZegoUIKit.getSignalingPlugin().sendInvitation(inviteeIDs, timeout, type, data, notificationConfig).then(({
        code,
        message,
        callID,
        errorInvitees
        ...
        zloginfo(`[CallInvitation]Send invitation success, errCode: ${code}, message: ${message}, errorInvitees: ${errorInvitees}`);
        if (inviteeIDs.length > errorInvitees.length) {
          ...
          this.onInvitationSent(navigation, callID, invitees, successfulInvitees, roomID, isVideoCall, callName, showWaitingPageWhenGroupCall, timeout);
          resolve();
        } else {
  ...
  onInvitationSent(navigation, callID, allInvitees, successfulInvitees, roomID, isVideoCall, callName, showWaitingPageWhenGroupCall, timeout) {
    zloginfo(`[onInvitationSent] callID: ${callID}, roomID: ${roomID}`);
    const localUser = ZegoPrebuiltPlugins.getLocalUser();
    CallInviteStateManage.addInviteData(callID, localUser.userID, successfulInvitees);
