...
class CallEventNotifyApp {
  ...
  init(callbacks) {
    ...
    this.eventCallbacks['onIncomingCallRejectedCauseBusy'] = callbacks.onIncomingCallRejectedCauseBusy;
    ...
    (0, _logger.zloginfo)(`[CallEventNotifyApp][init] succeed`);
  }
  isFunction(callback) {
    return typeof callback === 'function';
  }
  notifyEvent(event, ...params) {
    let callback = this.eventCallbacks[event];
    if (!this.isFunction(callback)) {
      (0, _logger.zloginfo)(`[CallEventNotifyApp][${event}] is not configured or the timing is too early`);
      return;
    }
    (0, _logger.zloginfo)(`[CallEventNotifyApp][${event}] notify will`);
    callback(...params);
    (0, _logger.zloginfo)(`[CallEventNotifyApp][${event}] notify succeed`);
  }
}
exports.default = CallEventNotifyApp;
_CallEventNotifyApp = CallEventNotifyApp;
_defineProperty(CallEventNotifyApp, "_instance", void 0);
//# sourceMappingURL=callevent_notify_app.js.map