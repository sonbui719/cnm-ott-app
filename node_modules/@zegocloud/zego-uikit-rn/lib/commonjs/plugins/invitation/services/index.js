...
// ------- live audio room ------
const _queryUsersInRoomAttributesIteration = (nextFlag = '', count = _queryCount) => {
...
const _queryUsersInRoomAttributesFully = (nextFlag, count) => {
  ...
  return new Promise((resolve, reject) => {
    _queryUsersInRoomAttributesIteration(nextFlag, count).then(({
...
const _updateCoreUserAndNotifyChanges = infos => {
...
const ZegoUIKitSignalingPluginImpl = {
  ...
  login: async (userID, userName) => {
    ...
    var token = '';
    if (_appSign === '') {
      token = await _ZegoUIKitInternal.default.getToken();
      ZegoUIKitSignalingPlugin.getInstance().registerPluginEventHandler('onRequireNewToken', 'callbackID', () => {
        return _ZegoUIKitInternal.default.getToken();
      });
    }
  ...
  sendInvitation: (invitees, timeout, type, data, notificationConfig) => {
    ...
    (0, _logger.zloginfo)(`[ZegoUIKitSignalingPluginImpl][sendInvitation] invitees: ${invitees}, timeout: ${timeout}, type: ${type}, data: ${data}, notificationConfig: ${JSON.stringify(notificationConfig)}`);
  ...
  onInvitationReceived: (callbackID, callback, from) => {
    if (!ZegoUIKitSignalingPlugin) {
      (0, _logger.zlogerror)(`[Plugins][invitation]Signaling plugin install error.`);
      return;
    }
    (0, _logger.zloginfo)(`[ZegoUIKitSignalingPluginImpl] onInvitationReceived, callbackID: ${callbackID}, from: ${from}`);
  ...
  onLoginSuccess: (callbackID, callback) => {
    ...
    ZegoUIKitSignalingPlugin.getInstance().registerPluginEventHandler('onLoginSuccess', callbackID, callback);
  },
  ...
  joinRoom(roomID) {
    ...
    return new Promise((resolve, reject) => {
      ZegoUIKitSignalingPlugin.getInstance().invoke('joinRoom', {
        ...
        try {
          (0, _logger.zloginfo)('[Plugins][invitation] Join room successfully, room:', roomID);
  ...
  leaveRoom: () => {
    ...
    return new Promise((resolve, reject) => {
      ZegoUIKitSignalingPlugin.getInstance().invoke('leaveRoom').then(async () => {
        try {
          (0, _logger.zloginfo)('[Plugins][invitation] Leave room successfully.');
          resolve(new _defines2.ZegoInvitationImplResult('', ''));
        } catch (error) {
          (0, _logger.zlogerror)(`[Plugins][invitation] Failed to resolve leave room.`, error);
        }
      }).catch(error => {
        (0, _logger.zloginfo)('[Plugins][invitation] Failed to leave room.');
        reject(error);
      });
    });
  },
  leaveAllRoom: () => {
    ...
    return new Promise((resolve, reject) => {
      ZegoUIKitSignalingPlugin.getInstance().invoke('leaveAllRoom').then(async () => {
        try {
          (0, _logger.zloginfo)('[Plugins][invitation] Leave all room successfully.');
          resolve(new _defines2.ZegoInvitationImplResult('', ''));
        } catch (error) {
          (0, _logger.zlogerror)(`[Plugins][invitation] Failed to resolve leave all room.`, error);
        }
      }).catch(error => {
        (0, _logger.zloginfo)('[Plugins][invitation] Failed to leave all room.');
        reject(error);
      });
    });
  },
  ...
  onUsersInRoomAttributesUpdated: (callbackID, callback) => {
    ...
    if (typeof callback !== 'function') {
      ...
      ZegoUIKitSignalingPlugin.getInstance().registerPluginEventHandler('usersInRoomAttributesUpdated', callbackID, ({
        infos,
        editor
      }) => {
  ...
  queryRoomProperties() {
    ...
    return new Promise((resolve, reject) => {
      ZegoUIKitSignalingPlugin.getInstance().invoke('queryRoomProperties').then(({
        roomID,
        roomAttributes
      }) => {
