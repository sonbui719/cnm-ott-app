...
function ZegoCancelInvitationButton(props) {
  ...
  const onButtonPress = () => {
    ...
    _services.default.cancelInvitation(invitees, data).then(({
      code,
      message,
      callID,
      errorInvitees
    }) => {
      (0, _logger.zloginfo)(`[Components]Cancel invitation success, callID: ${callID}, errorInvitees: ${errorInvitees}`);
      if (invitees.length > errorInvitees.length) {
        if (typeof onPressed === 'function') {
          ...
          errorInvitees.forEach(errorInviteeID => {
            const index = succInvitees.findIndex(inviteeID => errorInviteeID === inviteeID);
