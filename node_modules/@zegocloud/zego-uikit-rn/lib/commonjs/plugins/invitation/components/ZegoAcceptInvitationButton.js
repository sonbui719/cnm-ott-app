...
function ZegoAcceptInvitationButton(props) {
  ...
  const onButtonPress = () => {
    ...
    _services.default.acceptInvitation(inviterID, data).then(result => {
      // @ts-ignore
      (0, _logger.zloginfo)(`[Components]Accept invitation success, callID: ${result.data.callID}`);
      if (typeof onPressed === 'function') {
        (0, _logger.zloginfo)('[ZegoAcceptInvitationButton][acceptInvitation] execute onPressed will');
        // @ts-ignore
        onPressed({
          inviterID,
          callID: result.data.callID
        });
        (0, _logger.zloginfo)('[ZegoAcceptInvitationButton][acceptInvitation] execute onPressed succeed');
      }
    }).catch(({
      code,
      message
