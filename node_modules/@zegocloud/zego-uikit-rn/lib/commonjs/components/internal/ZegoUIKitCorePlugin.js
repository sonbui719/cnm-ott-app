...
const ZegoUIKitCorePlugin = {
  installPlugins: plugins => {
    (0, _logger.zloginfo)(`[ZegoUIKitCorePlugin] installPlugins, plugins: ${plugins}`);
    ...
    plugins.forEach((plugin, index) => {
      if (plugin.ZIMConnectionState) {
        if (_plugins.get('ZIM')) {
          ...
          (0, _logger.zloginfo)(`[ZegoUIKitCorePlugin][installPlugins]Plugin install success, plugin: ZIM, index: ${index}`);
        }
        ...
        if (_plugins.get('ZPNs')) {
          ...
          (0, _logger.zloginfo)(`[ZegoUIKitCorePlugin][installPlugins]Plugin install success, plugin: ZPNs, index: ${index}`);
          if (!_plugins.get('CallKit') && _reactNative.Platform.OS === 'ios') {
            try {
              let callKitPlugin = require('zego-callkit-react-native');
              _plugins.set('CallKit', callKitPlugin);
              (0, _logger.zloginfo)(`[ZegoUIKitCorePlugin][installPlugins] Plugin install success, plugin: CallKit, index: ${index}`);
            } catch (error) {
              (0, _logger.zlogwarning)(`[ZegoUIKitCorePlugin][installPlugins] Cannot resolve module zego-callkit-react-native`);
            }
          }
        }
