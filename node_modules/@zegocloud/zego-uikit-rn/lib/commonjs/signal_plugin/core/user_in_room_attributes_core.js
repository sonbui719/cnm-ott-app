...
class ZegoPluginUserInRoomAttributesCore {
  ...
  _registerEngineCallback() {
    _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().on('roomMemberAttributesUpdated', (zim, {
      roomID,
      infos,
      operatedInfo
    }) => {
  ...
  leaveAllRoom() {
    ...
    return new Promise((resolve, reject) => {
      _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().leaveAllRoom().then(() => {
        (0, _logger.zloginfo)(`[ZegoPluginUserInRoomAttributesCore]Leave all room successfully.`);
        this._resetDataForLeaveRoom();
        resolve(new _defines.default('', ''));
      }).catch(error => {
        (0, _logger.zlogerror)(`[ZegoPluginUserInRoomAttributesCore]Failed to leave all room, code: ${error.code}, message: ${error.message}`);
        reject(error);
      });
    });
  }
  ...
  queryUsersInRoomAttributes(config) {
    ...
    return new Promise((resolve, reject) => {
      _ZegoUIKitCorePlugin.default.getZIMPlugin().default.getInstance().queryRoomMemberAttributesList(this._roomBaseInfo.roomID, config).then(({
        roomID,
        infos,
