...
=======
  ...
  useEffect(() => {
    if (messages.length > 0) {
      ...
      // Nếu tin nhắn cuối cùng không phải của mình, có text và không phải sticker
      if (!isMe && lastMsg.text && !isStickerMessage(lastMsg.text)) {
        ...
        setSuggestions([]); // Ẩn gợi ý nếu mình vừa gửi tin nhắn
        if (msgIdentifier !== lastSuggestedMsgId) {
          setLastSuggestedMsgId(msgIdentifier);
        }
      }
  ...
  const fetchSuggestions = async () => {
    ...
    setSuggestions([]);
    try {
      // Lấy tối đa 5 tin nhắn gần nhất làm ngữ cảnh
            ...
            >
              ...
              {!isSuggesting && suggestions.map((suggestion, index) => (
                <Pressable 
                  ...
                  onPress={() => {
                    setInputText(suggestion); // Điền vào ô chat
                  }}
                >
